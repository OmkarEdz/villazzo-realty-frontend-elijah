				...
				<div className="meet-wrap">
					<div className="meet-left">
						<p className="meet-left-img">
							<Image
								loader={myLoader}
								src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${zadkielstuart.attributes.memberImageURL}`}
								className="header_image"
								layout="fill"
								alt="Villazzo"
							/> 
						</p>
						<h3 className="meet-left-name">{zadkielstuart.attributes.memberName}</h3>
						<p className="broker">{zadkielstuart.attributes.memberType}</p>
						<a className="number" href={`tel:${encodeURIComponent(zadkielstuart.attributes.contactNo)}`}>{zadkielstuart.attributes.contactNo}</a>
						<a className="email" href={`mailto:${encodeURIComponent(zadkielstuart.attributes.memberEmail)}`}>{zadkielstuart.attributes.memberEmail}</a>
					</div>
					<div className="meet-right">
						<p>{zadkielstuart.attributes.Content}</p>
					</div>
				</div>
...
export async function getStaticProps() {
  ...
  const [
	...
	fetchAPI("/zadkielstuart", { populate: "*" }),
